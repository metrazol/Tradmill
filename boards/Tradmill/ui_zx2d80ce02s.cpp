...
static lv_obj_t *_lbl_stop;   // label inside the stop/start button
static lv_obj_t *_lbl_safety;

// Arc spans 240° centred at the bottom: 150° → 30° (clockwise).
// ARC_MAX is derived from MAX_SPEED_MPH so it scales correctly when
// the config changes (e.g. recalibration).
...
void ui_init() {
    ...
    // Build widgets
    // Layout (240x320 screen, y=0 at top):
    //   ~10         Incline label
    //   ~15-215     Speed arc (210x210, centre at y=115)
    //   ~75-95      Speed value inside arc
    //   ~215        Time label
    //   ~216-260    Stop button (centred)
    //   ~266-310    Slow- / Fast+ buttons (left / right)
    ...
    // Large speed value inside the arc
...
void ui_update(float speed_mph, int incline, uint32_t elapsed_sec,
    ...
    // Stop button reflects current state
    if (running) {
        lv_label_set_text(_lbl_stop, "Stop");
        lv_obj_set_style_bg_color(_btn_stop, lv_color_hex(0xAA2222), 0);
    } else {
